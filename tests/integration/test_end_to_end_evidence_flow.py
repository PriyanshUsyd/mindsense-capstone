...
def test_state_a_produces_ineligible_packet_with_no_evidence():
    ...
    draft = _draft(
        packet, response_mode=ResponseMode.INSUFFICIENT_DATA,
...
def _draft(packet, response_mode, claim_ids_used, includes_uncertainty_statement, evidence_ids_referenced=None):
    ...
    return AssistantDraft(
        ...
        evidence_ids_referenced=evidence_ids_referenced or (packet.feature_window.feature_id,),
        text="synthetic test response",
...
def test_state_b_produces_partial_packet_with_value_but_no_statistical_evidence():
    ...
    assert packet.evidence is None
    assert packet.baseline.eligibility_status == EligibilityStatus.PARTIAL_DESCRIPTIVE_ONLY

    draft = _draft(
        packet, response_mode=ResponseMode.UNCERTAINTY,
        claim_ids_used=(ApprovedClaimId.OBSERVATION_OF_DEVIATION, ApprovedClaimId.UNCERTAINTY_DISCLOSURE),
...
def test_state_b_draft_missing_uncertainty_statement_is_rejected():
    ...
    packet = EvidencePacket(
        ...
        feature_window=_base_feature_window(coverage_ratio=0.6),
        baseline=PersonalBaseline(
            method="n/a", value=None, n_baseline_observations=1,
...
def test_state_c_produces_eligible_packet_with_full_evidence():
    ...
    draft = _draft(
        packet, response_mode=ResponseMode.NORMAL,
        claim_ids_used=(ApprovedClaimId.OBSERVATION_OF_DEVIATION, ApprovedClaimId.WITHIN_PERSON_ASSOCIATION, ApprovedClaimId.UNCERTAINTY_DISCLOSURE),
        includes_uncertainty_statement=True,
    )
    ok, reason = validate_draft(packet, draft)
    assert ok is True, reason
...
def test_gate_rejects_prohibited_claim_smuggled_via_wrong_response_mode():
    ...
    packet = EvidencePacket(
        ...
        baseline=PersonalBaseline(
            method="n/a", value=None, n_baseline_observations=0,
    ...
    draft = _draft(
        packet, response_mode=ResponseMode.INSUFFICIENT_DATA,
        claim_ids_used=(ApprovedClaimId.WITHIN_PERSON_ASSOCIATION,),  # not approved by this packet
        includes_uncertainty_statement=False,
    )
    ok, reason = validate_draft(packet, draft)
    assert ok is False
...
def test_gate_rejects_response_mode_outside_permitted_set():
    ...
    draft = _draft(
        packet, response_mode=ResponseMode.NORMAL,  # not in permitted_response_modes
        claim_ids_used=(ApprovedClaimId.NOT_ENOUGH_DATA,),
        includes_uncertainty_statement=False,
    )
    ok, reason = validate_draft(packet, draft)
...
def test_gate_accepts_fallback_modes_without_requiring_uncertainty_statement():
    ...
    draft = _draft(
        packet, response_mode=ResponseMode.GENERIC_FALLBACK,
...
def test_gate_rejects_on_packet_id_mismatch_directly():
    ...
    mismatched_draft = _AD(**{**draft.model_dump(), "packet_id": "some_other_packet_id"})
